...
import AddAutoReplyRuleModal, { type NewAutoReplyRulePayload } from './AddAutoReplyRuleModal';
...
function describeTrigger(r: ApiAutoReplyRuleRow): string {
  ...
  if (im.startsWith('__time__:')) {
    const t = im.replace('__time__:', '');
    if (t === 'weekend') return '时间段：周末';
    if (t === 'weekday') return '时间段：工作日';
    if (t === 'daily') return '时间段：每天';
    return `时间段：${t}`;
  }
  if (im) return `意图：${im}`;
  return '—';
...
export default function AutoReplyRulesPage() {
  ...
  const handleCreate = async (payload: NewAutoReplyRulePayload) => {
    if (!apiOn) return;
    setSavingModal(true);
    setError(null);
    try {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">基于时间、关键字、意图配置自动回复（后端规则表）</p>
          </div>
          <button
            type="button"
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex gap-2">
                      ...
                      <span className="font-medium text-slate-900">
                        {rule.markRepliedOnSend ? '发送后标记已回复' : '仅记录（不自动标记已回复）'}
                      </span>
                    </div>
                  </div>
                </div>
