...
2. **工单与会话的状态流转引擎（核心变更）**：
   ...
   - 完整的自动生命周期：
     - **新入站/新创建**：工单产生时为 `未读 (unread)`（列表展示未读提示；**H5/移动端**为头像右上角**数字角标**，PC 仍为姓名旁小圆点或加粗预览，见 **4.3.1**、**§7.3**）。
...
24. **H5 收件箱列表与未读角标（最新更新，2026-05）**：
    - **共用组件**：H5 工单列表与 PC 共用 **`InboxList`**；视口为移动端或 `listPaginationMode=infinite` 时启用 **`useInfiniteList`**（上拉加载、左滑快捷操作、四宫格筛选下拉）。
    - **未读数字角标**：列表左侧为买家姓名**首字圆形头像**；右上角红色角标展示未读条数（`1`～`99`，超过显示 **`99+`**，样式参考微信会话列表）。**不在头像上叠加平台色块角标**。
    - **未读条数来源**：`Ticket.unreadCount`（母系统会话同步写入 `unreadCount` / `buyerUnreadCount` / `unreadMessageCount`）；无同步值且 `messageProcessingStatus === unread` 或 `status === NEW` 时**兜底为 1**。标为已读/已回复时后端将 `unreadCount` 置 **0**；买家新入站消息 **`increment`**。
    - **搜索交互（H5）**：顶栏「工单」行仅保留**搜索图标**；点击后在**筛选条（平台 / 店铺 / 状态 / 超时）上方**展开搜索框，**不在顶栏标题区展开**。占位：**「搜索平台订单号、主题、买家…」**；支持 `enterKeyHint=search` 与关闭按钮。
    - **左滑操作**：`InboxListSwipeRow` 提供 **置顶**、**标为已读/未读**、**标为已回复/未回复**（与 PC 演示逻辑一致，写回 `messageProcessingStatus` / `unreadCount`）。
    - **列表第二行**：优先展示 **`lastInboundPreview`**（最近买家入站摘要，约 30 字截断），否则回退 `getTicketSubjectForDisplay`。

25. **H5 工单详情底栏与 AI 润色可配置（最新更新，2026-05）**：
    - **详情 Tab 底栏**：各业务 Tab（会话除外）采用固定 **`H5TabBottomBar`**（内容区可滚动 + 底栏主操作），与 PC 侧栏按钮能力对齐，例如：**AI 摘要**「开始分析 / 生成智能回复」、**订单**「添加备注」（切至会话并聚焦内部备注）、**物流**同步 17track、**发票**生成预览、**售后**提交/再次提交。
    - **会话输入条**：`H5ComposerBar` 支持翻译（平台语言）、AI 润色/模板/附件/内部备注切换；翻译结果与空内容提示走本地 **`composerNotice`** 条。
    - **AI 润色方案**：设置页 **`AiPolishSettingsPage`** + **`aiPolishPresetsStore`**（默认方案 + `localStorage` CRUD）；后端 **`POST /api/ai/polish`** 支持 **`promptBlock`** 自定义提示词；工单详情/H5 通过 **`AiPolishPicker`** 选择方案。
    - **移动端弹窗适配**：`InvoicePreviewModal`、`SubmitAfterSalesModal` 在 H5/`isMobile` 下为纵向全宽底栏；售后弹窗中「管理签收方/承运商/售后类型」在移动端提示 **请在 PC 设置 → 字段管理维护**。

### 3.2 开发与交付进度快照（截至 2026-05）
...
| **数据库（PostgreSQL）** | Schema 已定义，迁移需按环境执行 | Prisma 模型见 `backend/prisma/schema.prisma`；含多租户、`TenantSyncSettings` / `TenantSyncState`、工单消息去重（`motherMessageId`）、**Ticket.unreadCount**（收件箱未读角标，迁移 `20260516120000_ticket_unread_count`）、**Ticket 复合索引**（SLA / 收件箱排序）、**AgentSeat.userId** 可选关联 User、**Order / Ticket / AfterSalesRecord 软删除字段 `deletedAt`**、**OrderItem** 行表。生产部署前需 `prisma migrate deploy` 与连接池（见 `backend/docs/DB_OPTIMIZATION.md`）。 |
...
| **前端 · 工单/收件箱** | 联调 + 部分演示并存 | 列表/详情、翻译、AI 生成/润色、发票预览与发送、内部备注编辑删除等与后端或 Mock 组合；**联调时**仪表盘四卡跳转的 **`?filter=`** 已传入 **`GET /api/tickets`**，与 **`/api/dashboard/inbox-metrics`** 口径对齐（见 **3.1-23**、**4.2.4**）。**H5** 见 **3.1-24/25**、**§7**。 |
...
| **搜索框（PC）** | 标题区下方常显；左侧 `Search` 图标；占位 **「搜索平台订单号、主题、买家…」**。 | **实际过滤字段**：`getTicketSubjectForDisplay(ticket)`、`subject`、`subjectOriginal`、`channelId`、`orderId`、关联订单的平台订单号 (`platformOrderId`)、买家姓名；联调时可由父组件 **`listSearchQuery` + `GET /api/tickets?search=`** 服务端过滤（`listSearchServerBacked`）。 |
| **搜索（H5 / 移动端）** | 顶栏「工单」行 **搜索图标**；点击后在 **筛选条上方** 展开输入框（**不在** `p-4` 标题区展开）。 | 与 PC 同一 `searchQuery` state；展开时自动 `focus`；右侧 **X** 收起；有搜索词或展开时图标高亮橙色。详见 **3.1-24**、**§7.3**。 |
| **筛选条（H5）** | 列表上方横向四宫格：**平台 / 店铺 / 状态 / 超时**（`InboxFilterDropdown`）。 | 替代 PC 手风琴 + 高级筛选网格；与 `selectedPlatform` / `selectedShop` / `filterConversationStatus` / `filterSla` 绑定。 |
...
| **会话头像（H5）** | 左侧 **44×44** 圆形容器，展示买家姓名**首字**（`InboxListConversationAvatar`）。 | 无买家头像 URL 时的占位；**不**在头像右下角叠平台色块。 |
| **未读角标（H5）** | 头像右上角红色圆角数字徽标（`#FA5151`、白字）；`1`～`99`，超过 **`99+`**。 | 展示值 = `resolveTicketUnreadBadgeCount(ticket)`：优先 **`unreadCount`**，否则未读/NEW 兜底 **1**；已读/已回复为 **0** 不展示。 |
| **未读提示（PC）** | 姓名左侧 **1px 圆点** `rose-500/90`（移动端列表已改为数字角标，见上行）。 | 条件：`messageProcessingStatus === 'unread'` **或** `status === NEW`。 |
| **左滑快捷操作（H5）** | `InboxListSwipeRow`：**置顶**、**标为已读/未读**、**标为已回复/未回复**。 | 标为已读时 `unreadCount: 0`；标为未读时 `unreadCount: 1`；置顶状态存 **`inboxPinnedStore`（localStorage）**。 |
| **买家姓名** | `customers[customerId].name`，缺省 **「买家」**。 | 列表主标识（头像右侧第一行）。 |
| **相对时间** | `formatDistanceToNow(createdAt)` + 中文后缀（`date-fns` `zhCN`）。 | 新鲜度（第一行右侧）。 |
| **预览第二行** | 优先 **`lastInboundPreview`**（买家最近入站摘要，约 30 字）；否则 `getTicketSubjectForDisplay(ticket)`；`line-clamp-2`；有未读时 `font-medium`。 | 与翻译设置、后端列表预览字段联动。 |
...
**Ticket**：id, tenantId, channelId, customerId, orderId?, status, priority, sentiment, intent, messageProcessingStatus, **unreadCount**（母系统未读条数，列表角标）, subject, subjectOriginal?, slaDueAt, isImportant, isFavorite, isShared, tags[], assignedSeatId?, ownerUserId?, firstResponseAt?, resolvedAt?, createdAt, updatedAt, **deletedAt?**, externalId?（母系统会话 ID）。
...
4. **移动端 (H5) 工单客户端**：提供独立入口的 H5 前端（仅工单列表与详情），与 PC 共用后端 API；主管看大盘仍以 PC 为主，H5 聚焦一线跟单与 SLA 急救。

## 7. 移动端 (H5) 独立工单客户端

为满足一线客服在手机端**跟单与 SLA 急救**、与 PC **字段与能力对齐**的诉求，H5 采用**独立构建入口**（`VITE_APP_TYPE=h5`，规范见 `src/h5/h5UiSpec.ts`），非 PC 三栏布局的缩小版。

### 7.1 与 PC 的关系与入口

- **独立前端入口**：`npm run dev:h5`，默认 Vite 端口 **4002**；PC 联调常用前端 **4000**、后端 **4001**（见 `.env.api` / `backend/.env`）。同源代理 `/api`、`/ws`。
- **认证**：与 PC 共用登录与 `AuthProvider`。
- **路由壳 `H5Shell`**：底部 **`H5TabBar`** 三 Tab — **BI** (`/bi`)、**客服** (`/inbox`，含 `/ticket/:id` 高亮)、**我的** (`/me`)。进入工单详情 **`/ticket/:ticketId`** 时**隐藏底栏**，避免与输入条冲突。
- **模块范围**：以 **客服收件箱 + 工单详情** 为主；BI/我的为 H5 扩展页；**不提供** PC 全套设置/订单/客户独立导航。

### 7.2 工单列表（`H5MailboxPage` + 共用 `InboxList`）

- **列表数据**：与 PC 相同工单 API / Mock；移动端自动 **`useInfiniteList`**（上拉追加、IntersectionObserver 哨兵）。
- **顶栏（`p-4`）**：标题 **「工单」** + 图标：**搜索**、新建（演示）、母系统同步、排序；**搜索输入框不在此区域展开**。
- **搜索（见 3.1-24）**：点击搜索图标后，在下方 **「平台 / 店铺 / 状态 / 超时」筛选条之上** 展开全宽搜索框；占位 **「搜索平台订单号、主题、买家…」**；关闭后收起。
- **筛选**：`InboxFilterDropdown` 横向滚动四宫格（平台、店铺、会话状态、SLA 超时）；PC 端仍为手风琴 + 可选高级筛选网格。
- **列表行（微信式）**：
  - 左：**圆形头像**（买家首字）+ 右上角 **未读数字角标**（`unreadCount`，`99+` 封顶）；
  - 右：买家名 + 相对时间；第二行消息预览（`lastInboundPreview` 优先）；
  - **左滑**：置顶 / 标为已读·未读 / 标为已回复·未回复。
- **进入详情**：`/ticket/:ticketId`（与 PC `/mailbox/:id` 区分）。

### 7.3 工单详情与 Tab（`H5TicketDetail`）

详情顶栏 + **横向滑动业务 Tab**（下划线激活态，与 `h5UiSpec` 一致），分区与 PC 侧栏一致：

| Tab | 内容区 | 底栏 `H5TabBottomBar`（固定底栏，内容可滚动） |
| --- | --- | --- |
| **会话** | IM 气泡流 + `H5ComposerBar` | 发送、翻译、润色、模板、附件、内部备注（无单独底栏条，输入条即底栏） |
| **AI 摘要** | 分析结果区 | **开始分析**、**生成智能回复** |
| **订单** | 订单/客户字段 | **添加备注**（切至会话 Tab、`isInternalNote`、聚焦输入） |
| **物流** | 轨迹与时间轴 | **同步 17track** |
| **发票** | 预览与主体信息 | **生成并预览** / 维护主体（与 PC 能力对齐） |
| **售后** | 售后记录列表 | **提交售后** / **再次提交** |

- **发票 / 售后弹窗**：移动端全宽底栏按钮；售后字段管理类入口在 H5 提示 **PC 设置 → 字段管理**。
- **翻译反馈**：`composerNotice` 条展示成功/失败/空内容提示（H5 详情页不依赖全局 `setGlobalApiError`）。

### 7.4 会话 Tab 操作（须保留）

- **发送消息**（客户 / 平台经理）
- **翻译**（按渠道 `channelPlatformLanguage`；空草稿/内部备注有提示）
- **AI 智能生成** / **AI 润色**（润色方案见 **3.1-25**，`AiPolishPicker` + 设置页方案管理）
- **模板插入**、**附件**、**内部备注** 与对外回复切换

### 7.5 已废弃的 H5 方案说明（文档修订记录）

| 原草案（§7 历史 `<details>`） | 当前实现 |
| --- | --- |
| 底部 Tab：工作台 / 消息 / 我的 | **BI / 客服 / 我的**；工单在「客服」 |
| 顶栏固定搜索框 | **筛选条上方按需展开**（3.1-24） |
| 列表仅 1px 未读红点 | **头像右上角数字角标** + `unreadCount` |
| 详情仅订单/物流/售后三 Tab | **六 Tab** 与 PC 对齐（§7.3） |
| 移动端不提供发票 | **发票 Tab + 预览弹窗**（移动端布局适配） |

细则以 **3.1-24/25**、**4.3.1** 与代码为准；历史交互草案保留于文末 `<details>` 仅供对照。

---

**（以下为历史草案存档，仅供对照，以实现代码与 §7.1–7.4 为准）**

<details>
<summary>历史：原 §7 底部导航与模块取舍草案</summary>

为满足客服主管随时监控团队状态以及一线客服在非办公环境（如通勤、居家隔离等紧急情况）下处理高优工单、防止 SLA 超时的诉求，特设计 IntelliDesk H5 移动端版本。

### 7.1 移动端模块取舍与优先级分析（历史）

移动端屏幕空间有限、交互方式以点按和滑动为主，因此不追求功能的大而全，而是聚焦于**“看数据”**与**“急救场”**两个核心场景。

#### 适合搬到移动端的核心模块 (High Priority)
1. **工作台 (Dashboard)**：主管/客服随时查阅当日核心指标（未读、超时工单）、AI 效能洞察及在线坐席情况。
2. **工单收件箱 (Inbox/Tickets)**：提供精简版的工单列表与聊天室界面的工单详情，用于快速回复买家消息，阻断 SLA 超时风险。
3. **简化的业务看板 (订单/物流抽屉)**：在聊天界面提供快速查看订单摘要和最新物流状态的入口，辅助客服快速决策。

#### 仅供查看或简化的模块 (Medium Priority)
1. **售后管理 (After-sales)**：移动端主要提供列表浏览与状态确认，不再支持复杂的退款金额核算和多字段表单建单操作。
2. **订单/客户查询**：提供极其简化的列表，仅用于基础的模糊搜索和状态确认，去除横向滚动的复杂数据表格。

#### 不适合搬到移动端、予以剔除的模块 (Low Priority / Not Suitable)
1. **系统设置 (Settings)**：包含路由分配规则、自动翻译全局开关、知识库和 SLA 规则配置等。这些表单重、操作频率低，仅保留在 PC 端。
2. **复杂 PDF 与发票生成**：受限于移动端浏览器引擎（如 `html2canvas` 的排版和性能限制），移动端不提供发票预览与下发功能。
3. **复杂的模版/知识库内容维护**：移动端可**调用**已有模版或知识库，但不提供后台的 CRUD 增删改查。

### 7.2 H5 核心界面交互设计（历史）

整体采用典型的移动端底部导航栏 (Bottom Tab-bar) 架构，舍弃 PC 端的左右固定侧边栏。

#### 7.2.1 全局导航 (Layout)
- **底部导航栏 (Bottom Tab-bar)**：包含 3 个核心 Tab：
  - **工作台 (Home/Dashboard)**：对应数据大盘。
  - **消息 (Inbox)**：对应工单列表（带未读红点提示）。
  - **我的 (Profile)**：展示当前登录账号信息、简单偏好设置及退出登录。
- **全局浮层通知**：采用类似 App 的顶部系统通知条，推送临近超时的 SLA 告警消息。

#### 7.2.2 移动端工作台 (Mobile Dashboard)
- **纵向瀑布流布局**：所有模块自上而下排列，可纵向滑动。
- **头部欢迎区**：缩小占比，保留“在线坐席”状态指示小红点/小绿点。
- **AI 效能洞察**：以横向卡片展示，文字精简至两三行，支持点击展开查看全文。
- **四宫格数据卡片**：将 PC 端的横向 4 卡片改为 `2x2` 的网格布局（未读、已读未回复、已回复、超时）。点击对应卡片直接跳转到带特定 Filter 的“消息”Tab。
- **业务结构图表**：圆环图采用单列铺开展示（意图分布、情绪分布），图例统一放在图表下方，而非右侧。趋势折线图支持横屏全屏查看。

#### 7.2.3 移动端工单列表 (Mobile Inbox List)（历史草案，部分已由 §7.2 实现替代）
- **列表视图**：类似于微信的聊天列表视图。每行展示：
  - 左侧：客户姓名首字母头像，带未读红点。（**已实现为数字角标**，见 §7.2）
  - 中间：客户姓名（加粗）、最新消息摘要（单行截断）。
  - 右侧：相对时间（如 10分钟前）、SLA 超时状态徽标（极简颜色点或钟表图标）。
- **顶部检索与筛选**：固定在顶部的搜索框。下拉提供横向滑动的筛选 Chip（如：未回复、即将超时、全部平台）。
- **左滑快捷操作 (Swipe Actions)**：在列表项上向左滑动，可呼出“标为已读/未读”、“Snooze(稍后处理)”的快捷按钮。

#### 7.2.4 移动端会话详情 (Mobile Ticket Detail)
- **类似 IM 聊天界面**：进入详情后，隐藏底部导航栏，提供沉浸式的聊天体验。
- **顶部导航栏 (Navbar)**：
  - 左侧：返回箭头。
  - 中间：客户名称及平台图标。
  - 右侧：“汉堡菜单”或“资料”图标，点击右侧滑出抽屉（Drawer）查看订单/物流/售后信息。
- **中间会话区**：
  - 采用气泡对话流，左侧买家，右侧客服。
  - 上下双语结构（翻译开启时）在移动端需要调整字体大小以保证可读性。AI 气泡加上显著的紫色星星标识。
- **底部输入与工具栏**：
  - 默认收起状态：仅展示输入框和 `+` 号。
  - 点击 `+` 号弹出操作面板（半屏弹窗 Bottom Sheet），提供：**插入模版**、**AI 智能生成**、**发送图片附件**、**内部备注切换**。
  - AI 智能回复直接将结果上屏至输入框中供客服微调。

#### 7.2.5 业务辅助抽屉 (Mobile Context Drawer)
- 从会话界面右上角点击滑出。使用类似半屏 Modal (Bottom Sheet) 呈现。
- 采用 **顶部横向滑动 Tab**（订单、物流、售后）。
  - **订单 Tab**：纵向展示最核心的金额、状态、SKU 名称。去除母系统链接外跳（避免移动端跳失）。
  - **物流 Tab**：简化版垂直时间轴。
  - **售后 Tab**：仅展示当前处理中的售后单据状态（如“退款中”），不提供移动端发单能力。
