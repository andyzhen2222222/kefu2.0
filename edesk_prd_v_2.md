...
为满足一线客服在手机端**仅处理工单**、与 PC **字段与能力对齐**的诉求，H5 采用**独立构建入口**（非 PC 页面的简单缩放适配），界面只包含工单模块。

### 7.1 与 PC 的关系与入口

- **独立前端入口**：通过环境变量 `VITE_APP_TYPE=h5` 启用 H5 路由壳；本地开发脚本 `npm run dev:h5`，默认开发服务器端口 **4002**（与 PC 联调端口 4000 区分），同源代理 `/api`、`/ws` 至后端（与 `dev:api` 一致）。
- **认证**：与 PC 共用登录页与 `AuthProvider`；H5 内不提供仪表盘、设置、订单/客户独立菜单等 PC 模块路由。
- **模块范围**：H5 **仅**「工单列表 → 工单详情」；列表与详情的数据与接口与 PC 收件箱/工单详情一致。

### 7.2 工单列表

- **保留**与 PC 收件箱一致的**搜索**与**筛选**能力（渠道、意图、情绪、订单状态、会话状态、SLA 等），布局为移动端纵向 + 横向滑动筛选区。
- 点击列表项进入详情，路由形态为 `/ticket/:ticketId`（与 PC 的 `/mailbox/:ticketId` 区分，避免混用书签）。

### 7.3 工单详情与 Tab

详情页采用 **Tab** 切换（与 PC 侧栏信息分区一致、字段保持一致，复用同一套业务展示逻辑）：

1. **会话**：消息时间线、气泡展示；与 PC 一致的翻译展示策略。
2. **AI 摘要**：工单级 AI 分析、摘要、意图/情绪等与 PC 一致。
3. **订单**：客户与订单字段与 PC 侧栏订单区一致（含收货地址等由订单接口补全的逻辑）。
4. **发票**：与 PC 一致的发票预览与能力边界（演示/联调以实际接口为准）。
5. **物流**：与 PC 一致的物流展示与同步操作。
6. **售后**：与 PC 一致的售后记录与申请入口。

### 7.4 会话 Tab 操作（须保留）

在会话 Tab 内，须保留与 PC 对齐的客服操作能力：

- **发送消息**（含收件人：客户 / 平台经理，与 PC 一致）
- **翻译**（自动翻译配置与展示）
- **AI 自动回复**（智能草稿 / 建议弹窗等）
- **AI 润色**
- **模板插入**
...
原方案中的「移动端底部多 Tab（工作台 / 消息 / 我的）+ 详情内仅订单/物流/售后三 Tab」「移动端不提供发票」等描述**已由本节替代**：当前产品定位为 **H5 仅工单 + 六类 Tab 与 PC 字段一致**，发票与售后等以实际联调页面为准。
...
#### 7.2.3 移动端工单列表 (Mobile Inbox List)
