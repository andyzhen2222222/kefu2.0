...
router.get('/inbox-metrics', async (req: TenantRequest, res) => {
  try {
    ...
    console.error('[GET /api/dashboard/inbox-metrics]', e);
    res.status(500).json({
      error: 'dashboard_inbox_metrics_failed',
...
router.get('/structure', async (req: TenantRequest, res) => {
  try {
    ...
    for (const t of tickets) {
      ...
      const ck = platformGroupLabelForTicket(t.channel?.platformType, t.channel?.displayName);
    ...
    console.error('[GET /api/dashboard/structure]', e);
    res.status(500).json({
      error: 'dashboard_structure_failed',
      message: e instanceof Error ? e.message : String(e),
    });
  }
});
