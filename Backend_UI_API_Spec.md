...
| `/` | `DashboardPage` | 工作台 |
| `/mailbox`、`/mailbox/:ticketId` | `MailboxPage` | 收件箱 + 工单详情（同页左右栏） |
...
- **Message**：`id, ticketId, senderId, senderType ('customer'|'manager'|'agent'|'system'|'ai'), content, createdAt, isInternal?, translatedContent?, sentPlatformText?, deliveryTargets?`
...
**按钮**：主 **登录**（当前调 `signIn` Mock）、Google SSO / 微信 / 扫码（多数无真实逻辑）。

**需接口**：登录、验证码、图形码、SSO、会话续期、忘记密码。
...
- **AI 效能洞察**：挂载后调 `summarizeTicket`（Gemini 服务 Mock/真实配置）；展示生成文案。
- **使用导航**：4 个 `Link`（绑定店铺→`/settings`，配置坐席→`/settings/seats`，模版→`/settings/templates`，SLA→`/settings/sla`）。
- **四张指标卡**：未读 / 已读未回复 / 已回复 / 超时 → 链接 `/mailbox?filter=...`（**收件箱未读 query 参数**，需后端列表筛选对齐）。
...
### 3.3 收件箱 `/mailbox`（`MailboxPage` + `InboxList` + `TicketDetail`）
...
- **AI 草稿**：`generateReplySuggestion`（Gemini）。
...
3. **工单分配**：`TicketRoutingRulesBlock` — 规则表（优先级、平台、店铺、售后类型、分配坐席、启用）；**新增/编辑规则** 弹窗；删除。
...
| `generateReplySuggestion` | 工单详情 | AI 草稿 |
| `recognizeAfterSalesFromFeedback` | 售后弹窗 | AI 填表 |
| Gemini / API Key | `geminiService` | 由环境变量等配置 |
